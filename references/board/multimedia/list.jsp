...
today : <fmt:formatNumber value="${todayCnt}" pattern="#,###" />]</p>
	...
	<c:choose>
		...
		<c:when test="${empty boardList || fn:length(boardList) == 0}">
			<p class="nodata"><spring:message code="info.nodata" /></p>
		</c:when>
		<%-- 게시글이 있을경우 --%>
		<c:otherwise>
			<c:forEach var="listData" items="${boardList}" varStatus="status">
			<div class="siiruBoard-galleryBox">
				<%-- 썸네일이 있으면 썸네일 표출. 없으면 첨부파일 이미지를 썸네일로 표출 --%>
				<div class="photoBox">
					<c:choose>
						<%-- 뷰 권한이 있으면 --%>
						<c:when test="${boardAuth.viewAuth eq 'Y'}">
							<a href="<c:out value="${listData.href}" />"><img src="<c:out value="${listData.thumb}" />" alt="<c:out value="${listData.thumbAlt}" />"></a>
						</c:when>
						<%-- 뷰 권한이 없으면 --%>
						<c:otherwise>
				...
				<dl<c:if test="${listData.delDt eq 'Y'}"> class="throughline"</c:if>>
					<%-- 게시글 제목 및 정보 표출 --%>
					<dt<c:if test="${listData.newAt eq 'Y'}"> class="new"</c:if>>
					<c:choose>
						<%-- 뷰 권한이 있으면 --%>
						<c:when test="${boardAuth.viewAuth eq 'Y'}">
							...
							<c:choose>
								<c:when test="${not empty listData.boardSj or listData.boardSj ne ''}"><c:out value="${listData.boardSj}" /></c:when>
								<c:otherwise>&nbsp;</c:otherwise>
							</c:choose>
							<c:if test="${listData.comtCnt ne ''}"> (<c:out value="${listData.comtCnt}" />)</c:if>
							</a>
						</c:when>
						<%-- 뷰 권한이 없으면 --%>
						<c:otherwise>
					...
					<%-- 작성자 및 작성일 --%>
					<dd class="siiru-tr"><c:out value="${listData.userNm}" /> <span><c:out value="${listData.regDt}" /></span></dd>
				</dl>
			</div>
			</c:forEach>
		</c:otherwise>
